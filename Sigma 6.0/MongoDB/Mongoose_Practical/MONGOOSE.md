...
Mongoose provides multiple delete methods depending on your use case.
...
## 3) `findOneAndDelete()`
Finds one document and deletes it (returns deleted doc).

```javascript
const deleted = await Student.findOneAndDelete({ name: "Aman" });
```
...
```javascript
const deletedById = await Student.findByIdAndDelete("66f1c1a4e4d0f8a0e8b12345");
```
...
## 6) Remove collection (MongoDB native via Mongoose)
...
## Important Notes
- `deleteOne()` and `deleteMany()` return a delete result (`deletedCount`).
- `findOneAndDelete()` / `findByIdAndDelete()` can return the deleted document.
- Use filters carefully to avoid accidental mass delete.

## Schema Validation in Mongoose

Schema validation ensures only valid data is saved in MongoDB.
Mongoose runs validations before `save()` and can also run them on updates.
...
const studentSchema = new mongoose.Schema({
  ...
  course: { type: String, enum: ["BCA", "BBA", "B.Tech", "B.Sc"] }
});
```
...
By default, many update methods do not run validators automatically.
...
```javascript
try {
  await Student.create({ name: "A", age: 200 });
} catch (err) {
